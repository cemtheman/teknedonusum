...
def test_sidebar_uses_location_and_date_defined_solar_season():
  ...
  assert '"Solar sezon başlangıcı"' in INPUTS_SOURCE
  assert '"Solar sezon bitişi"' in INPUTS_SOURCE
...
def test_sidebar_no_longer_exposes_manual_sun_hours_or_season_length():
  ...
  assert '"Sezonluk planlanan operasyon / rota günü"' in INPUTS_SOURCE
